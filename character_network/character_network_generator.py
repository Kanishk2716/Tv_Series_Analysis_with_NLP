...
class CharacterNetworkGenerator():
    ...
    def generate_character_network(self ,   df):
        ...
        for row in df['ners']:
            ...
            for sentence in row :
                previous_entities_in_windows.append(list(sentence))
                previous_entities_in_windows = previous_entities_in_windows[-windows:]

                # Flatten 2-D list into 1-D list
                previous_entities_flattened = sum(previous_entities_in_windows , [])
    ...
    def draw_network_graph(self,relationship_df):
        ...
        net = Network(notebook = False , width = "100%" , height = "700px" , bgcolor = "#222222" , font_color = "white" , cdn_resources = "remote")
